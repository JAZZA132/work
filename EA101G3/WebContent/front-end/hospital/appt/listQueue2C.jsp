...
<jsp:useBean id="listQueue2C" scope="request" type="java.util.List<OptVO>" />
...
<meta http-equiv="refresh" content="10;URL=<%=request.getContextPath()%>/front-end/hospital/appt/queue2C.jsp">
<link rel="stylesheet"
...
<c:choose>
    <c:when test="${fn:length(listQueue2C)==0}">
       <i class="fas fa-exclamation-circle"></i>夜診已結束
    </c:when>
    <c:otherwise>
    <c:forEach var="optVO" items="${listQueue2C}">
    
        <i class="fas fa-user-md"></i>目前看診號${optVO.currentCount+1-(fn:length(listQueue2C))} <br><br><br><br><br>
    </c:forEach>
