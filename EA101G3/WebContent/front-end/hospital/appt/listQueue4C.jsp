...
<jsp:useBean id="listQueue4C" scope="request" type="java.util.List<OptVO>" />
...
<meta http-equiv="refresh" content="10;URL=<%=request.getContextPath()%>/front-end/hospital/appt/queue4C.jsp">
<link rel="stylesheet"
...
<c:choose>
    <c:when test="${fn:length(listQueue4C)==0}">
       <i class="fas fa-exclamation-circle"></i>夜診已結束
    </c:when>
    <c:otherwise>
    <c:forEach var="optVO" items="${listQueue4C}">
    
        <i class="fas fa-user-md"></i>目前看診號${optVO.currentCount+1-(fn:length(listQueue4C))} <br><br><br><br><br>
     </c:forEach>
