...
<jsp:useBean id="listQueue4" scope="request" type="java.util.List<OptVO>" />
...
<meta http-equiv="refresh" content="10;URL=<%=request.getContextPath()%>/front-end/hospital/appt/queue4.jsp">
<link rel="stylesheet"
...
<c:choose>
    <c:when test="${fn:length(listQueue4)==0}">
       <i class="fas fa-exclamation-circle"></i>早診已結束
    </c:when>
    <c:otherwise>
    <c:forEach var="optVO" items="${listQueue4}">
    
        <i class="fas fa-user-md"></i>目前看診號${optVO.currentCount+1-(fn:length(listQueue4))} <br><br><br><br><br>
     </c:forEach>
