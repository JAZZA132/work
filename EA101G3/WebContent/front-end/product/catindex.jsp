...
<link rel="stylesheet" href="<%= request.getContextPath()%>/front-end/product/catindex.css">
<!-- bootstrap、FontAwesome、googleFont -->
<style>
.myjumbotron {
	background-image: url("<%=request.getContextPath()%>/front-end/product/img/4.jpg");
		...
		<div class="nav-scroller py-1 mb-2">
			<nav class="nav d-flex justify-content-end mynavbar">
				...
				<a class="p-2  ml-2 dropdown" href="#" id="navbarDropdownMenuLink"
					data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">個人資料</a>
					<a class="p-2  ml-2 text-danger" href="<%=request.getContextPath()%>/front-end/product/shopcart.jsp">購物車</a>
				<div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
					    <a class="dropdown-item" href="<%=request.getContextPath()%>/front-end/product/myorder.jsp">查看歷史訂單</a> <a
						class="dropdown-item" href="<%=request.getContextPath()%>/front-end/product/shopcart.jsp">我的購物車</a> 
				</div>
			</nav>
		</div>
		<div
			...
			<div class="col-md-6 px-0">
				<h1 class="display-4 font-italic text-dark">歡迎來到貓咪專區</h1>
			...
			$(document).ready(function(){
				$("#btn").click(function(){
					$.ajax({
						...
						type:"POST",
						data:{action:"selectcat", productname: $("#productname").val() },
						dataType:"json",
